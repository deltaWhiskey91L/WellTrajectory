...
def survey(md, inc, azi):
    ...
    for i in range(1, len(md)):
        dP = next_pt([md[i - 1], md[i]], [inc[i - 1], inc[i]], [azi[i - 1], azi[i]])
        tvd.append(tvd[i - 1] + dP[0])
        north.append(north[i - 1] + dP[1])
        east.append(east[i - 1] + dP[2])
...
def unit_vector(inc, azi):
    ...
    wA = np.array([np.cos(inc), np.sin(inc) * np.cos(azi), np.sin(inc) * np.sin(azi)])
