...
def survey(md, inc, azi):
    """
    Calculates the survey for the entire well using the Balanced Tangential Method.
...
def next_pt(md2, inc2, azi2):
    ...
    dm = md2[1] - md2[0]
    dv = dm / 2 * (np.cos(inc2[0]) + np.cos(inc2[1]))
    dn = dm / 2 * np.sin(inc2[0]) * np.cos(azi2[0]) + dm / 2 * np.sin(inc2[1]) * np.cos(azi2[1])
    de = dm / 2 * np.sin(inc2[0]) * np.sin(azi2[0]) + dm / 2 * np.sin(inc2[1]) * np.sin(azi2[1])
